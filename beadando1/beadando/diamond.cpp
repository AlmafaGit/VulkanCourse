...
static constexpr size_t g_pedestalPerVertexItemCount = 3 + 2 + 3;
static constexpr size_t g_pedestalVertexSize         = sizeof(float) * g_pedestalPerVertexItemCount;
static constexpr size_t g_pedestalVertexCount        = sizeof(g_pedestalVertices) / g_pedestalVertexSize;
...
VkResult Diamond::Create(const Context& context, const VkFormat colorFormat, const uint32_t pushConstantStart)
{
    ...
    m_buffer = BufferInfo::Create(context.physicalDevice(), device, sizeof(g_pedestalVertices), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    m_buffer.Update(device, g_pedestalVertices, sizeof(g_pedestalVertices));

    m_vertexCount = g_pedestalVertexCount;

    return VK_SUCCESS;
}

void Diamond::Destroy(const VkDevice device)
{
...
void Diamond::Draw(const VkCommandBuffer cmdBuffer, bool bindPipeline)
{
