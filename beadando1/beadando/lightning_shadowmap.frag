...
float SimpleShadow(vec4 fragPosLightSpace) {
    ...
    if (currentDepth > 1.0 || currentDepth < -1.0f) {
        return 0.0f;
    }
...
void main() {
    ...
    // final result
    float shadow =
        PCF ? PCFShadow(in_fragPosLightSpace) : SimpleShadow(in_fragPosLightSpace);

    vec3 result = (ambient + (1.0 - shadow)) * diffuse;
    out_color = vec4(result * pixel.rgb, 1.0);
    //out_color = vec4(pixel.rgb, 1.0);
}
